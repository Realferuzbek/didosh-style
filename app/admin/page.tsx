...
export default function AdminPage() {
  ...
  if (isChecking) return <div className="min-h-screen bg-[#1A1218]" />
  if (!isAuthed)  return <LoginForm onSuccess={() => setIsAuthed(true)} />
      ...
      <main className="p-4 md:p-6 max-w-screen-xl mx-auto">
        <div className="mt-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {stats.map((stat) => (
              <div key={stat.label} className="bg-[#2C1F28] rounded-2xl p-4 border border-[#3D2A36]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <a href="/admin/products"
              ...
              <div className="w-12 h-12 bg-brand-deeprose/20 rounded-xl flex items-center justify-center text-2xl">👗</div>
              <div>
                <div className="text-white font-semibold text-base group-hover:text-brand-rose transition-colors">Mahsulotlarni boshqarish</div>
                <div className="text-[#9B7B85] text-sm mt-0.5">Qo&#39;shish, tahrirlash, o&#39;chirish</div>
              </div>
            ...
            <a href="/admin/orders"
              ...
              <div className="w-12 h-12 bg-brand-deeprose/20 rounded-xl flex items-center justify-center text-2xl">📋</div>
              <div>
                <div className="text-white font-semibold text-base group-hover:text-brand-rose transition-colors">Buyurtmalar</div>
                <div className="text-[#9B7B85] text-sm mt-0.5">Barcha buyurtmalarni ko&#39;rish</div>
              </div>
              <div className="ml-auto text-[#9B7B85]">→</div>
            </a>
          </div>
